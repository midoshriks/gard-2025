        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ display('Table' . ' ' . 'PepsiPlastic') }}</h3>
                </div>
                <div class="card-header">
                ...
                <div class="card-body">
                    <div class="table-responsive-sm">
                        <form action="{{ route('dashboard.delets.pepsiplastic') }}" method="post">
                            @csrf
                            @method('DELETE')
                                ...
                                <tbody>
                                    @foreach ($pepsiplastic as $key => $pepsipals)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>
                                                <input class="form-check-input ids" name="ids[{{ $pepsipals->id }}]"
                                                    type="checkbox" value="{{ $pepsipals->id }}">
                                            </td>
                                            {{-- <td>{{ $pepsipals->date_A }}</td> --}}
                                            <td>{{ Carbon\Carbon::parse($pepsipals->date_A)->format('d-M') }}</td>
                                            ...
                                            <td
                                                style="color: {{ $pepsipals->disability_J >= 0 ? 'rgb(114, 190, 26)' : 'red' }} ">
                                                <b>{{ $pepsipals->disability_J }}</b>
                                            </td>
                            ...
                            <div class="card-footer text-end">
                                ...
                                <input class="btn btn-success" type="button" onclick="deSelect()"
                                    value="Deselect All" />
                            </div>
